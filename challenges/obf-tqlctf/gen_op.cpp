...
void randmerge(std::string*l,std::string*r)
{
	int n=r-l;
	if(n==1)
	{
...
int main(int argc,char**argv)
{
	...
	fo0(i,m)v[i]=(v[i]+(ll)D*(-binop_and1(base[0].s[i],base[1].s[i])+P))%P;
	fo0(i,m)v[i]=(v[i]+(ll)E*(-binop_or(base[0].s[i],base[1].s[i])+P))%P;
	fo0(i,m)v[i]=(v[i]+(ll)F*(-binop_xorT(base[0].s[i],base[1].s[i])+P))%P;
	//g.pb(base[0].u);
	//g.pb(base[1].u);
	...
	randmerge(g.data(),g.data()+g.size());
