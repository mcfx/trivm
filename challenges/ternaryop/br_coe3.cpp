...
lf loss(lf a[n][m],lf b[n][m],lf s[m][n])
{
	lf res=0;
	fo0(i,n)fo0(j,n)
	{
		lf u[m]={},v[n]={};
		fo0(k,m)u[k]+=a[i][k]*b[j][k];
		fo0(k,m)fo0(l,n)v[l]+=u[k]*s[k][l];
		fo0(k,n)res+=sqr(v[k]-(k==g[i][j]));
	}
	return res;
}
...
int main()
{
	...
	//out,prec(4),loss(t),'\n';
	lf ta[n][m],tb[n][m],ts[m][n];
	fo0(i,n)fo0(j,m)ta[i][j]=tb[i][j]=ts[j][i]=1;
	...
	fo0(_,10000000)
	{
		...
		lf ua[n][m],ub[n][m],us[m][n];
		fo0(i,n)fo0(j,m)ua[i][j]=ta[i][j]+ranl()*step;
		fo0(i,n)fo0(j,m)ub[i][j]=tb[i][j]+ranl()*step;
		fo0(i,m)fo0(j,n)us[i][j]=ts[i][j]+ranl()*step;
		
		/*fo0(i,3)ua[i][3]=ub[i][3]=1;
		
		ua[0][0]=ua[1][0]=1,ua[2][0]=-2;
		ua[0][1]=2,ua[1][1]=-1,ua[2][1]=-1;
		ua[0][2]=-1,ua[1][2]=2,ua[2][2]=-1;
		
		ub[0][0]=-1,ub[1][0]=2,ub[2][0]=-1;
		ub[0][1]=-1,ub[1][1]=-1,ub[2][1]=2;
		ub[0][2]=2,ub[1][2]=-1,ub[2][2]=-1;*/
		
		/*fo0(i,3)fo0(j,2)ua[i][j]=ub[i][j]=1;
		ua[0][0]=ua[2][0]=ub[0][0]=ub[2][0]=-1;
		ua[1][2]=ub[1][2]=ua[1][3]=ub[1][3]=0;
		ua[0][2]=0;
		ua[0][3]=1;
		ua[2][3]=0;
		ua[2][2]=1;
		...
		fo0(i,3)fo0(j,1)ua[i][j]=ub[i][j]=1;
		ua[0][1]=ua[1][1]=-1,ua[2][1]=1;
		ub[0][1]=ub[1][1]=-1,ub[2][1]=1;
		ua[1][2]=ub[1][3]=-1;
		ua[1][3]=ub[1][2]=0;
		ua[0][2]=1;
		ua[0][3]=-1;
		...
		if(nxt<cur)
		{
			fo0(i,n)fo0(j,m)ta[i][j]=ua[i][j];
			fo0(i,n)fo0(j,m)tb[i][j]=ub[i][j];
			fo0(i,m)fo0(j,n)ts[i][j]=us[i][j];
			cur=nxt;
			//out,prec(5),cur,'\n';
		}
		step*=.999999;
	...
	out,prec(10),cur,'\n';
	fo0(i,n){fo0(j,m)out,prec(3),ta[i][j],' ';out,'\n';}out,'\n';
	fo0(i,n){fo0(j,m)out,prec(3),tb[i][j],' ';out,'\n';}out,'\n';
	fo0(i,m){fo0(j,n)out,prec(3),ts[i][j],' ';out,'\n';}out,'\n';
}
