...
void trans_34(int*a,ll*r,int n,int n4,const int g[3][4])
{
	...
	int m=n/3,m4=n4/4;
	fo0(i,3)trans_34(a+i*m,r+i*m4,m,m4,g);
	for(int i=0;i<m4;i++)
	{
		ll t[4]={};
...
template<int c>void itrans_43_ll(ll*r,int n,int n4,const int g[4][3])
{
	if(n==1)return;
	int m=n/3,m4=n4/4;
	fo0(i,4)itrans_43_ll<c>(r+i*m4,m,m4,g);
	for(int i=0;i<m;i++)
	{
		ll t[3]={};
...
template<int c>void itrans_43_128(lll*r,int n,int n4,const int g[4][3])
{
	if(n==1)return;
	int m=n/3,m4=n4/4;
	fo0(i,4)itrans_43_128<c>(r+i*m4,m,m4,g);
	for(int i=0;i<m;i++)
	{
		lll t[3]={};
		fo0(j,4)fo0(k,3)t[k]+=r[i+j*m4]*g[j][k];
...
{
	if(n<=4782969)return conv_xor1_fast(a,b,r,n);
	int n4=1,t=n;
	while(t>1)t/=3,n4*=4;
	ll*au=new ll[n4];
	ll*bu=new ll[n4];
...
{
	if(n<=4782969)return conv_xor_fast(a,b,r,n);
	if(n==1)
	{
		r[0]=umod(r[0]+(ll)a[0]*b[0]);
		return;
